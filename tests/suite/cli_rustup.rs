...
async fn show_active_toolchain_with_verbose() {
    let mut cx = CliTestContext::new(Scenario::SimpleV2).await;
    cx.config.expect_ok(&["rustup", "default", "nightly"]).await;
    // Updated to expect component information in verbose output
    cx.config
        .expect_stdout_ok(
...
path: "
    ...
    // This makes the test more resilient to component list changes
    cx.config
...
active because: directory override for '"
    ...
    // Check that the output contains compiler info
    cx.config
        .expect_stdout_contains(
            &["rustup", "show", "active-toolchain", "--verbose"],
    ...
    // Check that the output contains components
    cx.config
        .expect_stdout_contains(
            &["rustup", "show", "active-toolchain", "--verbose"],
            "\nInstalled components:",
